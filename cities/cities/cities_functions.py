from pyspark.sql.functions import col, split, substring, udf, when, min as ps_min, struct
...
def get_distance(lieu1, lieu2):
    return distance.distance(lieu1, lieu2).km

get_distance_udf = udf(get_distance)

def compute_distance(df):
    df = find_prefecture(df)
    df = df.withColumn("lat_gps", split(col("coordonnees_gps"),",")[0])\
            .withColumn("long_gps", split(col("coordonnees_gps"),",")[1])\
            .withColumn("lat_gps_prefecture", split(col("gps_prefecture"),",")[0])\
            .withColumn("long_gps_prefecture", split(col("gps_prefecture"),",")[1])
    df = df.withColumn("distance", get_distance_udf(struct("lat_gps", "long_gps"), 
                                                    struct("lat_gps_prefecture", "long_gps_prefecture")))                                           
    return df
