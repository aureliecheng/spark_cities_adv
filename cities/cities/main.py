from pyspark.sql.functions import substring, col, mean, expr
...
def main(spark):
  ...
  # mean
  magic_percentile = expr('percentile_approx(distance, 0.5)')
  dept = cities_dist.select("dept", "distance")\
                    .withColumn("mean_dist", mean("distance").over(Window.partitionBy("dept")))\
                    .withColumn("median_dist", magic_percentile.over(Window.partitionBy("dept")))\
                    .select("dept", "mean_dist", "median_dist")\
                    .distinct()
  Cities.write_csv(dept, '/data/refined/departement/v3/csv')
